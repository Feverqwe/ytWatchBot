...
class Youtube {
  ...
  clean() {
    return this.main.channels.removeUnusedChannels().then(() => {
      const Op = this.main.db.sequelize.Op;
      return this.main.db.models.Messages.destroy({
        where: {publishedAt: {[Op.lt]: this.getFullCheckTime(2)}}
      });
      /*const db = this.main.db;
   ...
   */
  videoIdInList(id) {
    return this.main.db.models.Messages.count({
      where: {id}
    }).then((count) => {
      return !!count;
    });
  ...
  insertItem(channel, chatIdList, id, snippet, contentDetails) {
    const previewList = Object.values(snippet.thumbnails).sort((a, b) => {
      return a.width > b.width ? -1 : 1;
    }).map(item => item.url);

    const data = {
      url: `https://youtu.be/${encodeURIComponent(id)}`,
      title: snippet.title,
      preview: previewList,
    ...
    const ISOLATION_LEVELS = this.main.db.sequelize.Transaction.ISOLATION_LEVELS;
    return this.main.db.sequelize.transaction({
      isolationLevel: ISOLATION_LEVELS.REPEATABLE_READ,
    }, (transaction) => {
      return this.main.db.models.Messages.create(item, {transaction}).then((message) => {
        const messageId = message.id;
        return this.main.msgStack.addChatIdsMessageId(transaction, chatIdList, messageId);
      });
    }).then(() => {
  ...
  getVideoList(_channelList, isFullCheck) {
    const getVideoIdsInfo = (channel, ytVideoIds, chatIdList) => {
      ...
      return getPage().then(() => {
        ...
        if (isChange) {
          return this.main.channels.updateChannel(channel.id, channel);
        }
      }).catch((err) => {
      ...
      const getPage = (pageToken) => {
        ...
        const requestPage = () => {
          return gotLimited('https://www.googleapis.com/youtube/v3/activities', {
            query: {
              part: 'contentDetails',
              channelId: this.main.channels.unWrapId(channelId),
              maxResults: 50,
              pageToken: pageToken,
        ...
        return requestPage().then((/*v3Activities*/ {body: responseBody}) => {
          ...
          items.forEach((item) => {
            ...
            const videoId = this.main.channels.wrapId(ytVideoId, this.name);
            if (ids.indexOf(videoId) === -1) {
              ids.push(videoId);
              idVideoIdMap[videoId] = ytVideoId;
            }
          });
          return this.main.msgStack.messageIdsExists(ids).then((exIds) => {
            ids.forEach((id) => {
    ...
    return Promise.resolve().then(() => {
      ...
      return parallel(10, _channelList, (channel) => {
        return this.main.users.getChatIdsByChannel(channel.id).then((chatIdList) => {
      ...
      this.main.events.emit('subscribe', _channelList);
    });
  }
  ...
  getChannelId(channelName) {
    return this.getChannelIdByUrl(channelName).catch((err) => {
      ...
      return gotLimited('https://www.googleapis.com/youtube/v3/search', {
        ...
        return this.main.channels.insertChannel(channelId, this.name, title, url).then((channel) => {
          return this.main.channels.getChannels([channel.id]).then((channels) => {
            return channels[0];
          });
        });
      });
