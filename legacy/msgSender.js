...
class MsgSender {
  ...
  getValidPhotoUrl(stream) {
    let requestLimit = this.main.config.sendPhotoRequestLimit || 10;

    const requestTimeoutSec = this.main.config.sendPhotoRequestTimeoutSec || 30;
  ...
  getPicId(chat_id, text, stream) {
    const sendingPic = () => {
      const uploadPhoto = (photoUrl) => {
        return this.main.bot.sendPhoto(chat_id, got.stream(photoUrl), {
          caption: text
        }, {
      ...
      const sendPhotoUrl = (photoUrl) => {
        return this.main.bot.sendPhoto(chat_id, photoUrl, {
          caption: text
        });
      };
      ...
      promise = this.messageIdRequestPicturePromise[messageId] = this.getPicId(chat_id, caption, data).then(...promiseFinally(() => {
        ...
        return Promise.resolve().then(() => {
          ...
          if (fileId) {
            ...
            return this.main.msgStack.setImageFileId(messageId, fileId);
          }
        }).then(() => msg);
      });
  ...
  send(chat_id, imageFileId, caption, text) {
    if (!imageFileId || !caption) {
      return this.main.bot.sendMessage(chat_id, text, {
        parse_mode: 'HTML'
      });
    } else {
      return this.main.bot.sendPhotoQuote(chat_id, imageFileId, {
        caption: caption
      });
    }
  }
