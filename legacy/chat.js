...
var Chat = function (/**Main*/main) {
  var _this = this;
  var bot = main.bot;
  this.main = main;

  var language = main.language;
  var events = main.events;
  var services = main.services;
  var serviceToTitle = main.serviceToTitle;
  ...
  textOrCb(/(.+)/, function (req, next) {
    ...
    if (req.message) {
      ...
      commands.forEach(function (entity) {
        ...
        _this.main.tracker.track(req.message.chat.id, 'command', command, req.message.text);
      });
    } else
    if (req.callback_query) {
      ...
      _this.main.tracker.track(msg.chat.id, 'command', command, msg.text);
    }
  });
  ...
  textOrCb(/\/add(?:\s+(.+$))?/, function (req) {
    ...
    _this.main.bot.sendMessage(chatId, msgText, options).then(function (msg) {
      return router.waitResponse({
        ...
        _this.main.tracker.track(req.message.chat.id, 'command', '/add', req.message.text);
  ...
  textOrCb(/\/setChannel/, function (req) {
    ...
    return bot.sendMessage(chatId, msgText, options).then(function (msg) {
      return router.waitResponse({
        ...
        _this.main.tracker.track(_req.message.chat.id, 'command', '/setChannel', _req.message.text);
  ...
  textOrCb(/\/cleanYoutubeChannels/, function (req) {
    ...
    const adminIds = _this.main.config.adminIds || [];
    if (adminIds.indexOf(chatId) === -1) {
      return bot.sendMessage(chatId, 'Deny');
    }

    return _this.main.users.getAllChannels().then(function (channels) {
      ...
      channels.forEach(function (channel) {
        promise = promise.then(function () {
          return _this.main.services.youtube.channelExists(channel).catch(function (err) {
            ...
            return _this.main.channels.removeChannel(channel.id);
          });
        });
      });
      return promise;
  ...
  var deleteChannel = function (req, channelId) {
    ...
    return _this.main.users.removeChannel(req.chat.id, channel.id).then(function () {
      return _this.main.users.getChannels(req.chat.id).then(function (channels) {
        if (channels.length === 0) {
          return _this.main.users.removeChat(req.chat.id, 'Empty channels');
        }
      });
    }).then(function () {
      return _this.main.language.channelDeleted.replace('{channelName}', channel.title);
    });
  };
