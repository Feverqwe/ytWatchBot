    ...
    return this.inProgress(() =>
      this.oneLimit(async () => {
        while (true) {
          ...
          await this.main.db
            .setChannelsSyncTimeoutExpiresAtAndUncheckChanges(channelIds)
            .then(() => {
              const filterFn = (rawVideoIds: string[]) => {
                ...
                return this.main.db.getExistsVideoIds(videoIds).then((existsVideoIds) => {
                  return arrayDifference(videoIds, existsVideoIds).map((id) =>
                    serviceId.unwrap(id),
                  );
                });
              };
                ...
                rawVideos.forEach((rawVideo) => {
                  ...
                  rawChannelIds.forEach((rawChannelId) => {
                    ...
                    if (!checkedChannelIds.includes(video.channelId)) {
                      debug(
                        'Video %s skip, cause: Channel %s is not exists',
                        video.id,
                        video.channelId,
                      );
                      return;
